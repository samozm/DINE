#' @useDynLib DCENt, .registration = TRUE
...
estimate <- function(X,y,Z,n0,k0,t0,algo=2,max_itr=200,convergence_cutoff=5*(10^(-5)),REML=FALSE,verbose=FALSE,timings=FALSE,n_fold=5,seed=1121,n_threads=1,threshold=NA)
{
  ...
  {  
    ...
    res <- estimate_DEbeta(X,y,masterZ,MAP,n0,k0,t0,threshold,max_itr,convergence_cutoff,REML,verbose,timings,n_fold=n_fold,custom_theta=custom_theta,n_threads=n_threads,seed=seed) 
    #a2.estimate_DEbeta(X,y,Z,n0,k0,t0,max_itr,covtype,idx)
    sigma <- res$sigma
  }
  else if(algo==1)
  {
    res <- estimate_all(X,y,masterZ,MAP,n0,k0,t0,max_itr,convergence_cutoff,REML,n_fold,timings=timings,verbose=verbose,seed=seed)
    sigma <- 0
...
#' Build MAP and masterZ matrices for DCENt
#' 
