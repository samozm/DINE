...
#' @param X fixed effect matrix for intercept, timepoints, OTUs - should be in order all times for node 1 for subject 1, all times for node 2 for subject 1, and so on
#' @param U fixed effect matrix for demographic variables - should be in order all times for node 1 for subject 1, all times for node 2 for subject 1, and so on
...
#' }
#' @export  
estimate <- function(X,U,y,Z,n0,k0,t0,algo=2,max_itr=200,convergence_cutoff=5*(10^(-5)),REML=FALSE,verbose=FALSE,timings=FALSE,n_fold=5,n_threads=1,threshold=NA)
{
  startTime <- proc.time()
  V_nonzeros_pct = 0
  if(algo==2)
...
#' }
#' @export
estimate_masterZ <- function(X,U,y,masterZ,MAP,n0,k0,t0,algo=2,max_itr=200,convergence_cutoff=5*(10^(-5)),REML=FALSE,verbose=FALSE,timings=FALSE,n_fold=5,n_threads=1,threshold=NA)
{
  startTime <- proc.time()
  V_nonzeros_pct = 0
  if(algo==2)
